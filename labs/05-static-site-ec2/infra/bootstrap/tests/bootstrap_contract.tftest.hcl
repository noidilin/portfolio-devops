    ...
    json = jsonencode({
      ...
      Statement = [{
        ...
        Condition = {
          StringEquals = {
            "token.actions.githubusercontent.com:aud" = "sts.amazonaws.com"
            "token.actions.githubusercontent.com:sub" = "repo:OWNER/REPO:environment:lab-05-stage"
          }
        }
      }]
    })
...
run "lab05_bootstrap_contract" {
  ...
  assert {
    condition     = strcontains(data.aws_iam_policy_document.github_image_push_assume_role.json, "repo:OWNER/REPO:ref:refs/heads/main") && strcontains(data.aws_iam_policy_document.github_apply_assume_role.json, "repo:OWNER/REPO:environment:lab-05-stage") && aws_iam_role.github_apply.assume_role_policy == data.aws_iam_policy_document.github_apply_assume_role.json
