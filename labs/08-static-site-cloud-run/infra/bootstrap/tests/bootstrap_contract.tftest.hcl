...
run "lab08_bootstrap_contract" {
  ...
  assert {
    condition     = google_storage_bucket_iam_member.plan_state_read.role == "roles/storage.objectViewer" && strcontains(google_storage_bucket_iam_member.plan_state_read.condition[0].expression, "gcp/bootstrap/labs/08-static-site-cloud-run/") && strcontains(google_storage_bucket_iam_member.plan_state_read.condition[0].expression, "gcp/runtime/labs/08-static-site-cloud-run/stage/") && strcontains(google_storage_bucket_iam_member.plan_state_lock_bootstrap.condition[0].expression, "gcp/bootstrap/labs/08-static-site-cloud-run/") && strcontains(google_storage_bucket_iam_member.plan_state_lock_runtime.condition[0].expression, "gcp/runtime/labs/08-static-site-cloud-run/stage/") && strcontains(google_storage_bucket_iam_member.plan_state_lock_runtime.condition[0].expression, ".tflock") && strcontains(google_storage_bucket_iam_member.apply_state_runtime.condition[0].expression, "gcp/runtime/labs/08-static-site-cloud-run/stage/") && !contains(google_project_iam_custom_role.plan.permissions, "storage.objects.update") && !contains(google_project_iam_custom_role.apply.permissions, "storage.objects.update")
    error_message = "GCP state IAM must allow scoped plan reads plus lock-only bootstrap/runtime mutations and apply runtime-only state access."
  }
